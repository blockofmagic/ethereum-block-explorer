  ...
  return (
    <li className="explorertransactionblock-container">
...

      {
         ...
         */
        !isHovering || !props.transaction || !props.transaction.hash ? null : (
              ...
              <div>
                ...
                <div className="explorertransactionblock-tooltip-value">
                  {props.transaction.from
                    ? `${props.transaction.from.slice(
                        0,
                        6
                      )}...${props.transaction.from.slice(-6)}`
                    : null}
                </div>
              </div>
              {/* Transaction Recipipent */}
              <div>
                <div className="explorertransactionblock-tooltip-field">TO</div>
                <div className="explorertransactionblock-tooltip-value">
                  {props.transaction.to
                    ? `${props.transaction.to.slice(
                        0,
                        6
                      )}...${props.transaction.to.slice(-6)}`
                    : null}
                </div>
              </div>
            </div>
                ...
                <div className="explorertransactionblock-tooltip-value">
                  ...
                  <div>
                    ...
                    <Web3Context.Consumer>
                      {({ state }: any) =>
                        parsedValue
                          ? `€${(parsedValue * state.ethPrice).toFixed(2)} @ €${
                              state.ethPrice
                            }`
                          : ""
                      }
