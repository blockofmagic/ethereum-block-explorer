...
const ExplorerContent = (props: ExplorerContentProps) => {
  ...
  const newestBlockNum = Object.keys(props.blocks)[
    Object.keys(props.blocks).length - 1
  ];
  ...
  const _blocks: (Block | {})[] = Object.values(props.blocks).filter(
    (block) =>
      block &&
  ...
  while (
    _blocks.length <
    props.blockRangeVisible[1] - props.blockRangeVisible[0]
  ) {
    _blocks.unshift({});
  }
    ...
    <section className="explorerviewcontent-container">
      {
         ...
         */
        Object.keys(props.blocks).length < 20 ? null : (
          <div
            className={`explorerviewcontent-show-newer-container ${
              needsUpdate ? "active" : ""
            }`}
          >
            <Button onClick={props.showNewerBlocks}>SHOW NEWER BLOCKS</Button>
          </div>
        )
      }
      <section className="explorerviewcontent-blocks">
        {
          ...
          _blocks.reverse().map((block: Block | any) => (
            <ExplorerBlockCard
              key={block.timestamp || Math.random()}
              {...{ block }}
            />
          ))
        }
        {
          ...
          Object.keys(props.blocks).length < 20 ? null : (
            <div className="explorerviewcontent-load-more-container">
              <Button onClick={props.getMoreBlocks}>LOAD MORE</Button>
            </div>
          )
        }
      </section>
